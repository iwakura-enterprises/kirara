...
public class HttpClientHttpCore extends HttpCore {
    ...
    public <T> CompletableFuture<T> send(ApiRequest<T> request) {
        ...
        getExecutor().execute(() -> {
            try {
                ...
                httpRequestBuilder.method(method, getBodyPublisher(kirara, body));
                ...
                final var response = convertBytesToResponse(kirara, httpResponse.body(), responseClass, responseHeaders);
     ...
     */
    protected HttpRequest.BodyPublisher getBodyPublisher(Kirara kirara, Object body) {
        if (body == null) {
            return HttpRequest.BodyPublishers.noBody();
        }

        return HttpRequest.BodyPublishers.ofByteArray(convertBodyToBytes(kirara, body));
    }
}
